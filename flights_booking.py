...
class BookingClass():
	...
	global fname,lname,src,dest,gender,date,msg,name,hash_name,screen

	def __init__(self,parent=None):
		global e1_val,e2_val,e3_val,e4_val,e5_val,e6_val,screen
		screen = tk.Toplevel()
	...
	def validate_date(self):
		global date, msg, screen
		date1_list = date.split('/')
		#print(date1_list)
		try:
			new_date1 = datetime(int(date1_list[2]), int(date1_list[1]), int(date1_list[0]))
		except Exception as e:
			fh1 = open("booking_logs.txt", 'a+')
			log_date = datetime.now().date()
			fh1.write(str(log_date) + str(e) + "\n")
			fh1.close()
			msg = "Error in date. Please Correct it."
			self.newDisplay()
		#print(new_date1)
	...
	def check_seat(self):
		...
		if f == 0:
			...
			fh.close()
			msg = name1 + " Booking Confirmed. \n Please make the payment at the baggage counter."
	...
	def newDisplay(self):
		...
		heading = tk.Label(screen1, text = msg, font=('aerial 16 bold'))
		heading.place(x=20,y=0)
